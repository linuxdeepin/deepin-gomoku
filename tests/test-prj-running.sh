...
make -j8
#生成asan日志和ut测试xml结果
./tests/deepin-gomoku-test --gtest_output=xml:./report/report_deepin-gomoku.xml
...
mv ./html/index.html ./html/cov_deepin-gomoku.html
#对asan、ut、代码覆盖率结果收集至指定文件夹
cp -r html ../$reportdir/
cp -r report ../$reportdir/
cp -r asan*.log* ../$reportdir/asan_deepin-gomoku.log
